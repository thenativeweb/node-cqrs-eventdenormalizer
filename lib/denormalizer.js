...
//  this.repository = repository(options.repository);
//
...
_.extend(Denormalizer.prototype, {
  ...
  defaultEventExtension: function (fn) {
    ...
    this.extendEvent = function (callback) {
      callback(null, fn());
    };
      ...
      async.parallel([
        ...
        function (callback) {
          if (self.onEventHandle) {
            ...
            self.onEventHandle(e, function (err) {
              if (err) {
                debug(err);
              }
              callback(err);
        ...
        if (callback) {
          callback(errs, evt, notifications);
        }
      });
