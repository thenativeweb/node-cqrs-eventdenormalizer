...
_.extend(Denormalizer.prototype, {
  ...
  handle: function (evt, callback) {
    ...
    var evtName = dotty.get(this.definitions.event.name);
    var evtPayload = dotty.get(this.definitions.event.payload);
    ...
    if (evtName === 'commandRejected' &&
      ...
      var info = {
        ...
        aggregate: !!this.definitions.event.aggregate ? dotty.get(this.definitions.event.aggregate) : undefined,
        context: !!this.definitions.event.context ? dotty.get(this.definitions.event.context) : undefined
      };
      
      if (!!this.definitions.event.aggregateId || !this.definitions.event.revision || !dotty.exists(this.definitions.event.revision)) {
        return this.onEventMissingHandle(info, evt);
      }

      this.revisionGuard.getRevision(evtPayload.reason.aggregateId, function(err, entry) {
        if (entry.revision - 1 < evtPayload.reason.aggregateRevision) {
          info.guardRevision = entry.revision;
