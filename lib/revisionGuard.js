...
function RevisionGuard (store, options) {
  ...
  var defaults = {
    ...
    prefix: 'default' // must be dafault - store also uses it
...
RevisionGuard.prototype = {
  ...
  queueEvent: function (evt, callback) {
    ...
    this.queue.push(concatenatedId, evtId, evt, callback, function (loopCount, waitAgain) {
      self.store.get(self.options.prefix, concatenatedId, function (err, revInStore) {
        if (err) {
          debug(err);
          // TODO: this might not work - no remove fn on the store
          self.store.remove(self.options.prefix, concatenatedId, evtId);
          return callback(err);
        }
  ...
  finishGuard: function (evt, revInStore, callback) {
    ...
    function next (data) {
      nextCalled = true;
      var concatenatedId = self.getConcatenatedId(evt);

      self.store.set(self.options.prefix, concatenatedId, data, revInEvt + 1, revInStore, function (err) {
        if (err) {
          debug(err);
        ...
        self.store.saveLastEvent(self.options.prefix, evt, function (err) {
          if (err) {
    ...
    var onBeforeSetMiddleware = (
      self.options &&
      self.options.middlewares &&
      self.options.middlewares.onBeforeSet) || null;

    if (!onBeforeSetMiddleware) {
      next(null);
    } else {
      onBeforeSetMiddleware(evt, next);
      if (nextCalled === false) {
