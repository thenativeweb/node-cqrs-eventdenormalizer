...
_.extend(Redis.prototype, {
  ...
  getValueOfId: function (id, callback) {
    this.client.get(id, (err, value) => {
      ...
      if (!value) {
        return callback('No value for the id:' + id);
      }
  ...
  getValueOfEachId: function (prefix, callback = (err, aggregateHandleFns) => {}) {
    ...
    function scanRecursive(curs) {
      self.client.scan(curs, 'MATCH', prefix + '::*', function (err, res) {
        ...
        function next() {
          // Check if we processed all ids from the redis store
          if (cursor === '0') {
        ...
        ids.forEach(id => {
          ...
          if (!uniqueIds[id] && id.indexOf('THE_LAST_SEEN_EVENT') === -1) {
            uniqueIds[id] = true;
